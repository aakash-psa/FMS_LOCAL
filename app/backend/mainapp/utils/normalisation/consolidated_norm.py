...
_ENABLE_FINANCIAL_STATEMENT_TRACE = True
...
def fninitialising_all_values(payload: Any, is_save=None) -> Optional[Any]:
    try:
        ...
        # Skip unwanted trace rows by line-item label before finalizing payload.
        def _norm_skip_text(value: Any) -> str:
            text = str(value or "").strip().lower()
            text = re.sub(r"[^a-z0-9]+", " ", text)
            return " ".join(text.split())

        _skip_targets = {_norm_skip_text(v) for v in _SKIP_LINE_ITEMS}
        _skip_mask = pd.Series(False, index=_financial_statement_trace_df.index)

        for _col in ("Line Item", "Line Item Name", "Main Category"):
            if _col in _financial_statement_trace_df.columns:
                _vals = _financial_statement_trace_df[_col].map(_norm_skip_text)
                _skip_mask = _skip_mask | _vals.isin(_skip_targets)

        if _skip_mask.any():
            _financial_statement_trace_df = _financial_statement_trace_df.loc[~_skip_mask].reset_index(drop=True)

        # Remove the unnecessary trace helper columns from the final payload.
        _financial_statement_trace_df = _financial_statement_trace_df.drop(
            columns=["No.", "Financial Statement", "Line Item Name"],
            errors="ignore",
        )
        ...
        _period_column = next(
            (
                col
                for col in _financial_statement_trace_df.columns
                if str(col).strip().lower() == "period"
            ),
            None,
        )
        if _period_column is not None:
            _financial_statement_trace_df = _financial_statement_trace_df.rename(
                columns={_period_column: "Period Start"}
            )

        if "Value Type" not in _financial_statement_trace_df.columns:
            insert_loc = (
                _financial_statement_trace_df.columns.get_loc("Period Start") + 1
                if "Period Start" in _financial_statement_trace_df.columns
                else len(_financial_statement_trace_df.columns)
            )
            _financial_statement_trace_df.insert(insert_loc, "Value Type", "Monthly")
        else:
            _financial_statement_trace_df["Value Type"] = "Monthly"

        # _financial_statement_trace_df.to_excel("financial_statement_trace.xlsx", index=False)
        import json
        normalized_dashboard_payload = json.loads(_financial_statement_trace_df.to_json(orient="records"))
