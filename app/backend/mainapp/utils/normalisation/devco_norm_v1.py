...
import warnings
import mainapp.utils.normalisation.landco_capex_module as landco_capex_module
...
def wrapper_for_vars(payload):
    ...
    try:
        ...
        landco_capex = landco_capex_module.fninitialising_all_values(payload)
        ...
        _global_tl_daf_cap = Global.AssetLevelFundingAssumptions.term_loan_assumptions_debt_arrangement_fees_capitalization == "Yes"
        _asset_tl_daf_cap_raw = Asset.Financing.debt_arrangement_fees_capitalized if Asset.Financing.debt_arrangement_fees_capitalized else [None] * _n_assets
        _is_tl_daf_cap_per_asset = np.array([
            (str(v).strip().lower() == 'yes' if (_override_mask[i] and v is not None and str(v).strip() != '') else _global_tl_daf_cap)
            for i, v in enumerate(_asset_tl_daf_cap_raw)
        ])
        _atl_arr_fees = _asset_tl_arr_fees_amt[:, np.newaxis] * _asset_tl_drawdown_flag * (~_is_tl_daf_cap_per_asset[:, np.newaxis])
        _atl_arr_fees_cap = _asset_tl_arr_fees_amt[:, np.newaxis] * _asset_tl_drawdown_flag * _is_tl_daf_cap_per_asset[:, np.newaxis]
        ...
        # Per-asset revolver interest & fees capitalization (override takes precedence over Global)
        _global_rev_cap = Global.AssetLevelFundingAssumptions.asset_debt_revolver_assumptions_interest_and_fees_capitalization == "Yes"
        _asset_rev_cap_raw = Asset.Financing.interest_and_fees_capitalized if Asset.Financing.interest_and_fees_capitalized else [None] * _n_assets
        _int_cap_per_asset = np.array([
            (str(v).strip().lower() == 'yes' if (_override_mask[i] and v is not None and str(v).strip() != '') else _global_rev_cap)
            for i, v in enumerate(_asset_rev_cap_raw)
        ])
        ...
        for col in range(_n_periods):
            ...
            _arr_fees_cap[:, col] = _arr_fee_amount * _int_cap_per_asset
            _arr_fees[:, col] = _arr_fee_amount * (~_int_cap_per_asset)
            ...
            _commit_fees_cap[:, col] = _commit_fee_amount * _int_cap_per_asset
            _commit_fees[:, col] = _commit_fee_amount * (~_int_cap_per_asset)
            ...
            _cap_interest[:, col] = _interest_amount * _int_cap_per_asset
            _exp_interest[:, col] = _interest_amount * (~_int_cap_per_asset)
        ...
        if _all_assets_included:
            ...
            _net_pf = _cfo_pf + _cfi_pf
            _v_proj_equity = np.zeros(len(_net_pf))
            _pf_running_bal = 0.0
            for _t in range(len(_net_pf)):
                _pf_running_bal += _net_pf[_t]
                if _pf_running_bal < 0:
                    _v_proj_equity[_t] = -_pf_running_bal
                    _pf_running_bal = 0.0
        ...
        try:
            ...
            _assetco_jv_inclusion_flag = (
                pd.Series(Asset.JVModule.assetco_inclusion)
                .eq("Yes")
                .reindex(o_sales_revenue.index, fill_value=False)
            )
            ...
            _devco_to_assetco_internal_transfer_mask = (
                _devco_inclusion_flag & _assetco_inclusion_flag & _devco_jv_inclusion_flag & _assetco_jv_inclusion_flag
            ).values[:, np.newaxis].astype(float)
            ...
            def _build_land_area_by_asset():
                    data = {
                        "asset_unique_identifier": _as_list(Asset.ProjectDetails.asset_unique_identifier),
                        "asset_name": _as_list(Asset.ProjectDetails.asset_name),
                        "region": _as_list(Asset.ProjectDetails.region),
                        "city": _as_list(Asset.ProjectDetails.city),
                        "asset_class": _as_list(Asset.ProjectDetails.asset_class),
                        "sub_category": _as_list(Asset.ProjectDetails.sub_category),
                        "typology": _as_list(Asset.ProjectDetails.typology),
                        "construction_phase": _as_list(Asset.ProjectDetails.construction_phase),
                        "business_model": _as_list(Asset.BusinessModel.funding_type),
                        "exit_counterparty": _as_list(Asset.BusinessModel.exit_counterparty),
                        "asset_order": _as_list(Asset.ProjectDetails.number),
                        "units": _as_list(Asset.LandAreaProgram.units),
                        "developable_land_area": _as_list(Asset.LandAreaProgram.developable_land_area),
                        "master_plan_efficiency": _as_list(Asset.LandAreaProgram.master_plan_efficiency),
                        "gross_land_area": _as_list(Asset.LandAreaProgram.gross_land_area),
                        "site_coverage_ratio": _as_list(Asset.LandAreaProgram.site_coverage_ratio),
                        "site_coverage_area": _as_list(Asset.LandAreaProgram.site_coverage_area),
                        "gross_floor_area": _as_list(Asset.LandAreaProgram.gross_floor_area),
                        "built_up_area": _as_list(Asset.LandAreaProgram.built_up_area),
                        "floor_plan_efficiency": _as_list(Asset.LandAreaProgram.floor_plan_efficiency),
                        "total_nsa__gla": _as_list(Asset.LandAreaProgram.total_nsa__gla),
                        "floor_area_ratio": _as_list(Asset.LandAreaProgram.floor_area_ratio),
                        "parking_bays": _as_list(Asset.LandAreaProgram.parking_bays),
                        
                        "devco_inclusion": _as_list(Asset.JVModule.devco_inclusion),
                        "jvjda_inclusion": _as_list(Asset.JVModule.jvjda_inclusion),
                        "venture_type": _as_list(Asset.JVModule.venture_type),
                        
                        "asset_devco_inclusion": _as_list(Asset.ProjectDetails.asset_devco_inclusion),
                        "sales_override": _as_list(Asset.Disposal.sales_override),
                        "sales_value": _as_list(Asset.Disposal.sales_value),
                    }
            ...
            def _build_model_assumptions_payload():
                return {
                    "ModelAssumptions": _safe_dict({
                        "model_start_date": Global.ModelAssumptions.model_start_date,
                        "model_end_date": Global.ModelAssumptions.model_end_date,
                    }),
                    "ValuationAssumptions": _safe_dict({
                        "npv_calculation_start_date": Global.ValuationAssumptions.npv_calculation_start_date,
                        "discount_rate": Global.ValuationAssumptions.discount_rate,
                        "mirr_finance_rate": Global.ValuationAssumptions.mirr_finance_rate,
                        "mirr_reinvestment_rate": Global.ValuationAssumptions.mirr_reinvestment_rate,
                        "irr_calculation_option": Global.ValuationAssumptions.irr_calculation_option,
                    }),
                    "ProjectInputs": _safe_dict({
                        "project_name": Global.ProjectInputs.project_name,
                        "project_start_date": Global.ProjectInputs.project_start_date,
                        "project_id": Global.ProjectInputs.project_id,
                        "location": Global.ProjectInputs.location,
                        "current_sg": Global.ProjectInputs.current_sg,
                        "approval_date": Global.ProjectInputs.approval_date,
                        "committee_approval": Global.ProjectInputs.committee_approval,
                        }),
                    "AssetLevelFundingAssumptions": _safe_dict({
                        "term_loan_assumptions_loan_term": Global.AssetLevelFundingAssumptions.term_loan_assumptions_loan_term,
                        "asset_debt_revolver_assumptions_revolver_facility_duration": Global.AssetLevelFundingAssumptions.asset_debt_revolver_assumptions_revolver_facility_duration,
                    }),
                    "ProjectLevelFundingAssumptions": _safe_dict({
                        "term_loan_assumptions_loan_term": Global.ProjectLevelFundingAssumptions.term_loan_assumptions_loan_term,
                        "project_debt_revolver_assumptions_revolver_facility_duration": Global.ProjectLevelFundingAssumptions.project_debt_revolver_assumptions_revolver_facility_duration,
                    }),
                    "SalesInputs": _safe_dict({
                        "sales_sales_cost_per_sqm": Global.SalesInputs.sales_sales_cost_per_sqm,
                    })
                }
            # return excel_output 
            if "Value Type" in normalized_dashboard_data.columns:
                    normalized_dashboard_data = normalized_dashboard_data[
                        normalized_dashboard_data["Value Type"] != "Annual"
                    ].reset_index(drop=True)
            # normalized_dashboard_data.to_excel("DC.xlsx",index= False)
            import json
            normalized_dashboard_payload = json.loads(normalized_dashboard_data.to_json(orient="records"))
            return {
                "ModelAssumptions": _build_model_assumptions_payload(),
                "InputDetails": _build_land_area_by_asset(),
...
def fninitialising_all_values(payload):
    try:
        # import json
        # with open("devco_payload.json", "w", encoding="utf-8") as f:
