    ...
    try:
            ...
            try:
                ...
                data_registry = {
                    ...
                    "Land in Kind": {
                        "df": _apply_asset_filter(o_land_acquisition_cost_in_kind),
                        "Model": "LandCo",
                        "Cashflow Section": "Cashflow from Financing",
                ...
                def _format_epoch_date(v):
                    ...
                    try:
                        if isinstance(v, (int, float, np.integer, np.floating)) and not isinstance(v, bool):
                            ...
                            if _str_v == "":
                                return v
                            if _str_v.lstrip("-").isdigit():
                                _epoch_val = float(_str_v)
                            else:
                                _dt = pd.to_datetime(v, errors="coerce")
                                return v if pd.isna(_dt) else _dt.strftime("%Y-%m-%d")
                        else:
                            _dt = pd.to_datetime(v, errors="coerce")
                            return v if pd.isna(_dt) else _dt.strftime("%Y-%m-%d")

                        _abs_epoch = abs(_epoch_val)
                        if _abs_epoch >= 1e12:
                            _dt = pd.to_datetime(_epoch_val, unit="ms", errors="coerce")
                        elif _abs_epoch >= 1e9:
                            _dt = pd.to_datetime(_epoch_val, unit="s", errors="coerce")
                        else:
                            _dt = pd.to_datetime(_epoch_val, errors="coerce")
                ...
                def _build_land_area_by_asset():
                    data = {
                        ...
                        "business_model": _as_list(Asset.BusinessModelModule.business_model), 
                        "exit_counterparty": _as_list(Asset.BusinessModelModule.exit_counterparty), 
                        "units": _as_list(Asset.LandAreaModule.units),
                        "developable_land_area": _as_list(Asset.LandAreaModule.developable_land_area),
                        "master_plan_efficiency": _as_list(Asset.LandAreaModule.master_plan_efficiency),
                        "gross_land_area": _as_list(Asset.LandAreaModule.gross_land_area),
                        "site_coverage_ratio": _as_list(Asset.LandAreaModule.site_coverage_ratio),
                        "site_coverage_area": _as_list(Asset.LandAreaModule.site_coverage_area),
                        "gross_floor_area": _as_list(Asset.LandAreaModule.gross_floor_area),
                        "built_up_area": _as_list(Asset.LandAreaModule.built_up_area),
                        "floor_plan_efficiency": _as_list(Asset.LandAreaModule.floor_plan_efficiency),
                        "total_nsa__gla": _as_list(Asset.LandAreaModule.total_nsa__gla),
                        "floor_area_ratio": _as_list(Asset.LandAreaModule.floor_area_ratio),
                        "parking_bays": _as_list(Asset.LandAreaModule.parking_bays),
                        
                        "jvjda_inclusion": _as_list(Asset.JVModule.jvjda_inclusion),
                        "landco_inclusion": _as_list(Asset.JVModule.landco_inclusion),
                        "venture_type": _as_list(Asset.JVModule.venture_type),
                        
                        "land_bank_override": _as_list(Asset.LandBankModule.land_bank_override),
                        "holding_period": _as_list(Asset.LandBankModule.holding_period),
                        "asset_landco_inclusion": _as_list(Asset.ProjectDetails.asset_landco_inclusion),

                        "land_sales_price": _as_list(Asset.LandSalesModule.sales_price),
                        "land_sales_override": _as_list(Asset.LandSalesModule.land_sales_override),
                        "secondary_infrastructure_override": _as_list(Asset.SecondaryInfrastructureModule.override),
                        "primary_infrastructure_module_override": _as_list(Asset.PrimaryInfrastructureModule.override),
                        "secondary_infrastructure_module_construction_start_date": [
                            _format_epoch_date(v)
                            for v in _as_list(Asset.SecondaryInfrastructureModule.construction_start_date)
                        ],
                        "primary_infrastructure_module_construction_start_date": [
                            _format_epoch_date(v)
                            for v in _as_list(Asset.PrimaryInfrastructureModule.construction_start_date)
                        ],
                        "land_development": _as_list(Asset.AssetLifecycleModule.land_development),
                        "vertical_development": _as_list(Asset.AssetLifecycleModule.vertical_development),
                        "asset_operations": _as_list(Asset.AssetLifecycleModule.asset_operations)
                    }
                ...
                def _build_model_assumptions_payload():
                    return {
                        ...
                        "InfrastructureCosts": _safe_dict({
                            "primary_infrastructure_start_date": Global.InfrastructureCosts.primary_infrastructure_start_date,
                            "secondary_infrastructure_start_date": Global.InfrastructureCosts.secondary_infrastructure_start_date,
                        }),
                        ...
                        "FinancingAssumptions": _safe_dict({
                            "project_term_loan_loan_term": Global.FinancingAssumptions.project_term_loan_loan_term,
                            "project_term_loan_debt_drawdown_date": Global.FinancingAssumptions.project_term_loan_debt_drawdown_date,
                            "asset_debt_revolver_revolver_facility_duration": Global.FinancingAssumptions.asset_debt_revolver_revolver_facility_duration,
                            "project_debt_revolver_revolver_facility_duration": Global.FinancingAssumptions.project_debt_revolver_revolver_facility_duration,
                        }),
                        "LandBankInputs": _safe_dict({
                            "holding_period": Global.LandBankInputs.holding_period,
                        }),
                        "LandSalesInputs": _safe_dict({
                            "sales_price": Global.LandSalesInputs.sales_price,
                        }),
                    }
                # return excel_output
                ...
                # normalized_dashboard_data.to_excel("LC.xlsx",index=False)
                import json
                normalized_dashboard_payload = json.loads(
                    normalized_dashboard_data.to_json(orient="records")
                )
                return {
