...
def wrapper_for_vars(payload):
    ...
    try:
            ...
            financing_map = {
                ...
                "Equity Infusion": w_equity_infusion.iloc[0, :] * _project_level_multiplier if _all_assets_included else np.maximum((_inverse_filtered_sum(_apply_asset_filter(w_additional_requirement)) * -1), 0.0),
            ...
            try:
                ...
                # Placed here so elim variables (_o_on_plan_sales_collection_elim etc.)
                # are already defined. Basis: Net CFO + Net CFI - Land in Kind (negated if positive).
                _pa_cfo_cfi_items = [
                    # ── CFO ─────────────────────────────────────────────────
                    (_apply_asset_filter(o_on_plan_sales_collection),                                 +1),
                    (_apply_asset_filter(o_cash_inflow_from_off_plan_sales),                          +1),
                    (_apply_asset_filter(o_escrow_setup_fees),                                         -1),
                    (_apply_asset_filter(o_land_lease_revenue_including_grace_period),                +1),
                    (_apply_asset_filter(o_land_lease_associated_operating_expenses),                  -1),
                    (_apply_asset_filter(o_leasing_cost),                                              -1),
                    (_apply_asset_filter(o_sales_transaction_cost),                                    -1),
                    (_apply_asset_filter(o_other_income_1),                                           +1),
                    (_apply_asset_filter(o_other_income_2),                                           +1),
                    (_apply_asset_filter(o_other_income_3),                                           +1),
                    (_apply_asset_filter(o_other_expense_1),                                           -1),
                    (_apply_asset_filter(o_other_expense_2),                                           -1),
                    (_apply_asset_filter(o_other_expense_3),                                           -1),
                    (_apply_asset_filter(o_community_operations_cost_payment_amanah),                  -1),
                    (_apply_asset_filter(o_community_operations_amanah_recovery),                     +1),
                    (_apply_asset_filter(o_community_operations_cost_payment_roshn),                   -1),
                    (_apply_asset_filter(o_community_operations_roshn_recovery),                      +1),
                    (_apply_asset_filter(o_corporate_overheads_salaries_expensed),                     -1),
                    (_apply_asset_filter(o_corporate_overheads_it_services_expensed),                  -1),
                    (_apply_asset_filter(o_corporate_overheads_office_rent_and_utilities_expensed),    -1),
                    (_apply_asset_filter(o_corporate_overheads_professional_services_expensed),        -1),
                    (_apply_asset_filter(o_corporate_overheads_marketing_expensed),                    -1),
                    (_apply_asset_filter(o_corporate_overheads_sales_cost_expensed),                   -1),
                    (_apply_asset_filter(o_corporate_overheads_additional_expenses_expensed),          -1),
                    # ── CFI ─────────────────────────────────────────────────
                    (_apply_asset_filter(o_land_acquisition_cost_cash_payment),                        -1),
                    (_apply_asset_filter(o_acquisition_transaction_cost_legal),                        -1),
                    (_apply_asset_filter(o_acquisition_transaction_cost_agency),                       -1),
                    (_apply_asset_filter(o_acquisition_transaction_cost_technical),                    -1),
                    (_apply_asset_filter(o_acquisition_transaction_cost_valuation),                    -1),
                    (_apply_asset_filter(o_acquisition_transaction_cost_due_diligence),                -1),
                    (_apply_asset_filter(o_acquisition_transaction_cost_real_estate_transaction_tax),  -1),
                    (_apply_asset_filter(o_acquisition_transaction_cost_municipal_fees),               -1),
                    (_apply_asset_filter(o_primary_infrastructure_cost_payment),                       -1),
                    (_apply_asset_filter(o_secondary_infrastructure_cost_payment),                     -1),
                    (_apply_asset_filter(o_design_cost_payment),                                       -1),
                    (_apply_asset_filter(o_permitting_cost_payment),                                   -1),
                    (_apply_asset_filter(o_supervision_cost_payment),                                  -1),
                    (_apply_asset_filter(o_project_management_cost_payment),                           -1),
                    (_apply_asset_filter(o_contingency_cost_payment),                                  -1),
                    (_apply_asset_filter(o_corporate_overheads_salaries_capitalized),                  -1),
                    (_apply_asset_filter(o_corporate_overheads_it_services_capitalized),               -1),
                    (_apply_asset_filter(o_corporate_overheads_office_rent_and_utilities_capitalized), -1),
                    (_apply_asset_filter(o_corporate_overheads_professional_services_capitalized),     -1),
                    (_apply_asset_filter(o_corporate_overheads_marketing_capitalized),                 -1),
                    (_apply_asset_filter(o_corporate_overheads_sales_cost_capitalized),                -1),
                    (_apply_asset_filter(o_corporate_overheads_additional_expenses_capitalized),       -1),
                    (_apply_asset_filter(o_land_lease_exit_value),                                    +1),
                    (_apply_asset_filter(o_land_bank_exit_value),                                     +1),
                ]
                _pa_net_cf = fn_replace_nan(
                    sum(sign * df for df, sign in _pa_cfo_cfi_items), 0.0
                )
                _pa_lik = fn_replace_nan(_apply_asset_filter(o_land_acquisition_cost_in_kind), 0.0)
                _pa_net_cf = fn_replace_nan(
                    _pa_net_cf - _pa_lik.clip(lower=0) + _pa_lik.clip(upper=0), 0.0
                )
                _pa_net_cf.index = asset_name.values

                # ── Sales inclusion debug ────────────────────────────────────────────────
                _dbg_on_plan  = fn_replace_nan(_apply_asset_filter(o_on_plan_sales_collection),       0.0)
                _dbg_off_plan = fn_replace_nan(_apply_asset_filter(o_cash_inflow_from_off_plan_sales), 0.0)
                _dbg_on_plan.index  = asset_name.values
                _dbg_off_plan.index = asset_name.values
                print(f"[Asset Net CF] on-plan sales total per asset (no elim):\n{_dbg_on_plan.sum(axis=1)}")
                print(f"[Asset Net CF] off-plan sales total per asset (no elim):\n{_dbg_off_plan.sum(axis=1)}")
                print(f"[Asset Net CF] net CF total per asset:\n{_pa_net_cf.sum(axis=1)}")
                print(f"[Asset Net CF] _pa_net_cf shape={_pa_net_cf.shape}, any non-zero={(_pa_net_cf != 0).any().any()}")
                # ────────────────────────────────────────────────────────────────────────

                excel_output["monthly_dfs"]["Asset Net Cashflows"] = (
                    "o.landco.asset.net.cf.me", fn_df_to_json_with_index(_pa_net_cf)
                )
...
def fninitialising_all_values(payload):
    try:
            # ── Validation: only run for supported sensitivity / goal-seek types ──
            _ALLOWED_TYPES = {"Infrastructure Cost", "Land Sales Price"}
            ...
            _sensitivity_types = _get_payload_values("a.landco.sensitivity")
            _goal_seek_types   = _get_payload_values("a.landco.goal.seek")
            ...
            if _can_run_sensitivity:
                from mainapp.utils.v3.sensitivity_utils.landco_sensitivity import (
                    fn_run_landco_sensitivity,
                    fn_strip_sensitivity_result,
                )
                from mainapp.utils.v3.sensitivity_utils.common import (
                    export_sensitivity_to_excel,
                )
                raw_results = fn_run_landco_sensitivity(payload)
                # print(raw_results.keys())
                if raw_results is None:
                    print(f"[sensitivity] fn_run_landco_sensitivity returned None — skipping sensitivity, goal seek may still run")
                    raw_results = []

                # Build sensitivity section (all 25 scenarios stripped to Net CF + Sales)
                sensitivity_section = [
                    {
                ...
                # export_sensitivity_to_excel(sensitivity_section, "landco_sensitivity_export.xlsx")
                ...
                for _k, _item in enumerate(sensitivity_section, 1):
                    ...
                    if not isinstance(_res, dict):
                        continue
                    try:
                        from mainapp.utils.v3.sensitivity_utils.period_map import PERIOD_DATES
                        _, _json_df = _res["exceloutput"]["monthly_dfs"]["Asset Net Cashflows"]
                        _index = [("" if isinstance(i, float) else i) for i in _json_df["index"]]
                        _columns = [
                            PERIOD_DATES[c] if isinstance(c, int) and 0 <= c < len(PERIOD_DATES) else c
                            for c in _json_df["columns"]
                        ]
                        _df = _pd.DataFrame(
                            data=_json_df["data"],
                            index=_index,
            ...
            # export_sensitivity_to_excel(raw_results, "landco_sensitivity_export.xlsx")

            if _can_run_goal_seek:
                from mainapp.utils.v3.sensitivity_utils.goal_seek import fn_run_landco_goal_seek_iterative
                ...
                if base_result is None:
                    ...
                    goal_seek_result = fn_run_landco_goal_seek_iterative(payload, base_result)
                    if goal_seek_result is None:
                        print(f"[goal seek] fn_run_landco_goal_seek_iterative returned None — returning sensitivity data only")
            else:
            ...
            if goal_seek_result is None and not all_records:
                print(f"[landco] Both goal_seek and all_records are empty — returning None")
                return None
